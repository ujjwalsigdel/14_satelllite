...
```{r}
boundary <- read_sf("../../05_digitizing/output/boundary.shp") %>%
  st_transform(32616)
...
```{r}
rastlist <- list.files(path = "../data/classcotton-PSScene-ortho_analytic_4b_sr-2020-106-2020-274/",
...
```{r}
mapview(filter(allimages_dup, imageid == 3)$image_w) +
  mapview(filter(allimages_dup, imageid == 4)$image_w) 

```

### 2. Closest to solar noon  
```{r}
mapview(filter(allimages_dup, imageid == 12)$image_w) +
  mapview(filter(allimages_dup, imageid == 13)$image_w)
```
...
```{r}
pixels_f <- pixels %>%
  filter(!date %in% as.Date(c("2020-07-03", "2020-08-10")))
...
ggsave("../output/2020_gndvi_median.png",
       width = 4,
       height = 6,
       bg = "white")
...
```{r}
pixel_median %>%
  write_sf("../data/2020_pixel_median.geojson",
           delete_dsn = T
           )
