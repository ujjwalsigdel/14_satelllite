...
```{r }
bbox <- read_sf("../../05_digitizing/output/boundary.geojson") %>% 
  extent()
...
date_start <- as.Date("2017-04-15")
date_start

date_end <- as.Date("2017-09-30")
date_end
```
...
```{r}
# Metadata filters
cloud_lim <- .2  # percent from 0-1

item_name <- "PSScene"  # (see https://developers.planet.com/docs/data/items-assets/)

asset <- "ortho_analytic_4b_sr"  # (see https://developers.planet.com/docs/data/items-assets/)
...
```{r}
dir.create(file.path("../data/", exportfolder), 
           showWarnings = F)
...
```{r}
# Set API
api_key <- "PLAK28d19b9b94c7436fb1a4a89f3dbb78b2"
